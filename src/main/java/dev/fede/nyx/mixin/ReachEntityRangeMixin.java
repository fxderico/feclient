...
public class ReachEntityRangeMixin {
   @ModifyReturnValue(
      method = {"getEntityInteractionRange()D"},
      at = {@At("RETURN")}
   )
   private double nyx$extendEntityReach(double original) {
      if (!ReachModule.isEnabled_s()) {
         return original;
      } else {
         MinecraftClient var3 = MinecraftClient.getInstance();
         if (var3 != null && var3.player == ((Object)this)) {
            double var4 = ReachModule.getDouble();
