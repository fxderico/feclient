...
import dev.fede.zenithgui.client.screens.menu.settings.impl.popup.MenuEntityPopupSetting;
...
public class MenuEntitySetting extends MenuSetting {
    private final EntitySelectSetting setting;
    private Rect bounds;
    private ChangeRect boundsColor;

    public MenuEntitySetting(EntitySelectSetting setting) {
    ...
    public void render(
        ...
        ctx.drawText(Fonts.ICONS.getFont(6.0F), "E", settingX + 1.5F, iconY + 1.0F, themeColor);
    ...
    public void onMouseClicked(double mouseX, double mouseY, MouseButton button) {
        if (this.bounds != null && this.bounds.contains(mouseX, mouseY)) {
            FeClientBridge.getInstance().getMenuScreen().addPopupMenuSetting(new MenuEntityPopupSetting(this.setting, this.boundsColor));
        }
    }
